  ...
  return (
    <>
      ...
      {classNames.map(className => {
        ...
        const classLevel = firstEntry?.class.level ?? 'FUND2';
        const classShift = firstEntry?.class.shift ?? 'MORNING';
      ...
      >
        {editingSlot && (
          <>
            <p style={{ marginBottom: '1.5rem', color: 'var(--text-secondary)' }}>
              Alterar o professor de <strong>{editingSlot.subject.name}</strong> da turma{' '}
              <strong>{editingSlot.class.name}</strong>. Isso atualizará a Grade Curricular desta turma para este professor.
            </p>
